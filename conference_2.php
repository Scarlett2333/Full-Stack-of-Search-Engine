...
<html>
<head>
<title>Conference page conference</title>
<style type="text/css">
	body {
		text-align: center;
	}
...
<body>
	...
	<?php
		...
		{
		$wherelist[] = "conference id like '%".$conf_id."%";
		$urllist[] = "conference id = ".$conf_id;
		}
		...
		$result = mysqli_query($link, "SELECT PaperID,Title,PaperPublishYear from papers where ConferenceID='$conf_id' order by Title ASC");
		...
		<style>
				...
				Ranking:
				<div class="dropdown">
				  <button class="dropbtn">Order by Title(ASC)</button>
		...
		$result = mysqli_query($link, "SELECT PaperID,Title,PaperPublishYear from papers where ConferenceID='$conf_id' order by PaperPublishYear {$limit}");
		6.3new*/
		...
		if ($result) {
		 	...
		 	echo "<br\>";
		 	echo "<br\>";
		 	echo " 当前{$page}/{$maxpage}页 共{$total}条";
			echo " <a href='conference.php?page=1{$url}'>首页</a> ";
			echo "<a href='conference.php?page=".($page-1)."{$url}'>上一页</a>";
			echo "<a href='conference.php?page=".($page+1)."{$url}'>下一页</a>";
			echo " <a href='conference.php?page={$maxpage}{$url}'>尾页</a> ";
		 	6.3new*/
		 	...
		 	if ($total<=10){
                ...
                echo "<br \>";
                $char="Next Page";
                echo "<a href=\"/fanye_conf2.php?conf_id=$conf_id&total=$total&times=$times&page=$page\">$char </a>";
                $char="Back to Search";
                echo "<a href=\"/index.php?\">$char </a>";
                echo "<br \>";
                echo "<br \>";
